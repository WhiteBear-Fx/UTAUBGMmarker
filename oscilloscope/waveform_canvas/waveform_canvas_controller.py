...
class WaveformCanvasController:
    ...
    def draw_waveform(self):
        ...
        分析音频数据，并在示波器画布上显示波形。
        """
        width = self.view.get_canvas_info()[0]
        self._draw_waveform(*self.get_position(width))  # 重新分析音频数据并更新示波器显示

    def _draw_waveform(self, y1, y2):
        """
        分析音频数据，并在示波器画布上显示波形。
        """
        self.view.show_waveform(y1, y2)

    def get_position(self, width: int) -> tuple:

        # 获取画布的宽度和高度信息，用于适配音频数据的显示
        canvas_width, canvas_height = self.view.get_canvas_info()

        # 从音频加载器中获取与画布宽度相匹配的音频样本
        self.audio_data = self.audio_loader.get_audio_data(width)
        ...
        if self.audio_data is not None:
            ...
            # 使用处理好的波形数据更新示波器显示
            return waveform_top, waveform_bottom
    ...
    def on_resize(self):
        if self.audio_data is not None:  # 如果音频数据存在
            self._draw_waveform(*self.get_position(400))  # 绘制调整信息
